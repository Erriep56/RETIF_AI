...
Le dossier `[1_conversion_garnier](https://github.com/Erriep56/RETIF_AI/blob/main/1_conversion_garnier/conversion_rdf_json_txt.py)` contient un script Python permettant de convertir le fichier XML-RDF du **thésaurus Garnier (INHA)** en deux formats plus facilement exploitables : JSON et TXT.
...
Le dossier `2_projet_clip` contient :
- **Deux notebooks Jupyter** :
  - Le **notebook 1** (`1_recup_images_AGORHA.ipynb`) permet le téléchargement des images du RETIF via l’API d’AGORHA
  - Le **notebook 2** `2_embeddings_CLIP.ipynb` sert à générer les embeddings des images à l’aide du de la technologie CLIP
  
- Un fichier python `app.py` permettant de lancer l'**application Streamlit** pour la recherche visuelle d’images à partir de texte et d'images, en s’appuyant sur les embeddings CLIP générés par le Notebook 2.
